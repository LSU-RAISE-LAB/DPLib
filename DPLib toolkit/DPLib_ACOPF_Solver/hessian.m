...
if ngens~=0
VarVal1=PuScale*rho_theta*sigma;VarVal2=PuScale*rho_V*sigma;
else
VarVal1=rho_theta*sigma;VarVal2=rho_V*sigma;
end


for i=1:regvar
  H(region_buses{i}+nbuses,region_buses{i}+nbuses) =H(region_buses{i}+nbuses,region_buses{i}+nbuses)+(VarVal1/theta_scale)/theta_scale;
  H(2*nbuses+2*ngens+(2*i),2*nbuses+2*ngens+(2*i)) =H(2*nbuses+2*ngens+(2*i),2*nbuses+2*ngens+(2*i))+(VarVal1/theta_scale)/theta_scale;
  H(region_buses{i} ,region_buses{i}) =H(region_buses{i} ,region_buses{i})+(VarVal2/V_scale)/V_scale;
  H(2*nbuses+2*ngens+(2*i-1),2*nbuses+2*ngens+(2*i-1)) =H(2*nbuses+2*ngens+(2*i-1),2*nbuses+2*ngens+(2*i-1))+(VarVal2/V_scale)/V_scale;
end
