...
remote_nit  = loadedData.(['nit' remoteRegionStr]);
        ...
        if localRegion_num == region1_num
            grad_theta      = (x(region_buses{i} + nbuses)              - remotePhase(remote_nit + con(i)))/theta_scale;
            grad_theta_dual = (x(2*nbuses + 2*ngens + 2*i)           - remotePhase(con(i)))/theta_scale;
            grad_mag        = (x(region_buses{i})                   - remoteMag(remote_nit + con(i)))/V_scale;
            ...
            grad_theta_dual = (remotePhase(remote_nit + con(i))     - x(region_buses{i} + nbuses))/theta_scale;
            grad_mag        = (remoteMag(con(i))                    - x(2*nbuses + 2*ngens + 2*i - 1))/V_scale;
            grad_mag_dual   = (remoteMag(remote_nit + con(i))       - x(region_buses{i}))/V_scale;
        end
if ngens~=0
        % Augmented Lagrangian accumulation
        f = f + PuScale * ( ...
              landa(abs(y))            * grad_theta      + (rho_theta / 2) * grad_theta^2 + ...
              landa(abs(y) + nitall)      * grad_theta_dual + (rho_theta / 2) * grad_theta_dual^2 + ...
              lambdav(abs(y))          * grad_mag        + (rho_V / 2) * grad_mag^2 + ...
              lambdav(abs(y) + nitall)    * grad_mag_dual   + (rho_V / 2) * grad_mag_dual^2 );
else
        f = f + ( ...
              landa(abs(y))            * grad_theta      + (rho_theta / 2) * grad_theta^2 + ...
              landa(abs(y) + nitall)      * grad_theta_dual + (rho_theta / 2) * grad_theta_dual^2 + ...
